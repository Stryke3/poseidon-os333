...
async def upload_remittance(
    ...
    async with get_db_transaction() as conn:
        ...
        denial_count = 0
        for claim in claims:
            # Resolve order_id from patient_control_number (ORD00000123 → 123)
            pcn = claim.get("patient_control_number", "")
            resolved_order_id = _resolve_order_id(pcn)

            # Look up claim_submission if exists
            submission_id = None
            if resolved_order_id:
                submission_id = await conn.fetchval(
                    "SELECT id FROM claim_submissions WHERE order_id=$1 ORDER BY id DESC LIMIT 1",
                    resolved_order_id,
                )
            ...
            rc_id = await conn.fetchval("""
                ...
                RETURNING id
            """,
                batch_id, org_id, pcn, resolved_order_id,
    ...
    # Feed denials to Trident training pipeline (async, non-blocking)
    # This would call ml-service or write to payment_outcomes table
    if denial_count > 0:
...
async def auto_post_batch(batch_id: int):
    """
    ...
    async with get_db_transaction() as conn:
        batch = await conn.fetchrow(
            "SELECT * FROM remittance_batches WHERE id=$1", batch_id
        )
        if not batch:
            raise HTTPException(404, "Batch not found")

        claims = await conn.fetch(
            "SELECT * FROM remittance_claims WHERE batch_id=$1 AND auto_posted=FALSE", batch_id
        )
        ...
        for claim in claims:
            ...
            try:
                ...
                await conn.execute("""
                    INSERT INTO payment_outcomes
                        (claim_number, payment_status, paid_amount,
                         denial_code, payer_name, service_date,
                         source, created_at)
                    VALUES ($1, $2, $3, $4, $5, $6::DATE, 'era_835', NOW())
                    ON CONFLICT DO NOTHING
                """,
                    claim["patient_control_number"],
                    "denied" if claim["is_denial"] else "paid",
                    paid,
                    None,  # denial code aggregated from adjustments
                    batch["payer_name"],
                ...
                errors.append({"claim_id": claim["id"], "error": str(e)[:200]})
        ...
        await audit_log(conn, "remittance_batch", batch_id, "auto_posted",
...
async def get_batch_detail(batch_id: int):
    ...
    async with get_db() as conn:
        batch = await conn.fetchrow("SELECT * FROM remittance_batches WHERE id=$1", batch_id)
        if not batch:
            raise HTTPException(404, "Batch not found")

        claims = await conn.fetch(
            "SELECT * FROM remittance_claims WHERE batch_id=$1 ORDER BY id", batch_id
        )
...
@router.get("/stats")
async def remittance_stats(org_id: Optional[int] = None, days: int = Query(30, ge=7, le=365)):
    """Remittance KPIs for executive dashboard."""
    org_filter = "AND rb.org_id=$2" if org_id else ""
    params = [days, org_id] if org_id else [days]
...
def _resolve_order_id(patient_control_number: str) -> Optional[int]:
    """Extract order_id from our patient control number format: ORD00000123 → 123."""

...
export default function LearningRecommendationsPage() {
  ...
  const loadQueue = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await fetch(`${API}/governance/recommendations?status=PENDING`)
  ...
  const decide = useCallback(
    async (verb: "approve" | "reject") => {
      ...
      setError(null)
      try {
        const res = await fetch(`${API}/governance/${encodeURIComponent(id)}/${verb}`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ notes: decisionReason.trim() || undefined }),
        })
        const json = await res.json().catch(() => ({}))
        if (!res.ok) {
          setError(typeof json?.error === "string" ? json.error : `HTTP ${res.status}`)
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <h1 className="font-display text-3xl uppercase tracking-[0.1em] text-white">Recommendations</h1>
      <p className="text-sm text-slate-300/95">
        Queue via <code className="text-slate-300">/governance/recommendations</code>; evaluate via{" "}
        <code className="text-slate-300">/evaluate</code>; decisions via{" "}
        <code className="text-slate-300">/governance/:id/approve|reject</code>.
      </p>
      ...
      <section className="space-y-2 glass-panel rounded-[20px] p-4">
        ...
        <input
          ...
          placeholder="Optional notes (audit)"
        />

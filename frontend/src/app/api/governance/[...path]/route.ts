...
const GOVERNANCE_PREFIX = "/api/intelligence/governance"

function governanceUpstream(pathSegments: string[], search: string): string {
  const base = availityServiceBaseUrl().replace(/\/$/, "")
  const sub = pathSegments.map((s) => encodeURIComponent(s)).join("/")
  return `${base}${GOVERNANCE_PREFIX}/${sub}${search}`
}
...
async function proxy(req: Request, pathSegments: string[]): Promise<Response> {
  ...
  const upstream = governanceUpstream(pathSegments, url.search)

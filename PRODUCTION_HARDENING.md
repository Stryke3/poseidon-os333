...
This runbook is intentionally DO-only and does not include Render or edge-hosted frontend paths.
...
This validates compose syntax, pinned containers, Python syntax, and frontend production build.
...
## 4. Big-bang DNS cutover (remove all Render pointers)

At DNS provider:

1. Replace all `*.onrender.com` CNAME targets for live POSEIDON hosts.
2. Point `api`, `dashboard`, `trident`, `intake`, `ml`, `edi` to the DigitalOcean droplet target.
...
```bash
bash scripts/audit_no_render_pointers.sh
```
...
After DNS is confirmed to no longer point to Render and endpoints are healthy through DO:

- Suspend or delete Render services for POSEIDON.
- Confirm no production hostname resolves to `onrender.com`.

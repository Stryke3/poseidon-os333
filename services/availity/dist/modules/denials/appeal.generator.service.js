...
exports.generateAppealDraft = generateAppealDraft;
 ...
 */
function generateAppealDraft(params) {
    ...
    const getDocExcerpt = (docType, maxLen) => {
    ...
    if (denial.denialCode)
        letterSections.push(`DenialCode: ${denial.denialCode}`);
    ...
    for (const f of classification.requiredFixes)
        letterSections.push(`- ${f}`);
    letterSections.push("");
    ...
    if (evidenceExcerpts.length === 0) {
        ...
        for (const e of evidenceExcerpts)
            letterSections.push(`- ${e}`);
    }
    letterSections.push("");
    letterSections.push(`Escalation steps (deterministic):`);
    for (const s of classification.escalationSteps)
        letterSections.push(`- ${s}`);
...
//# sourceMappingURL=appeal.generator.service.js.map